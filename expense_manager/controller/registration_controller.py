...
class RegistrationController:
    ...
    def registration(self, username, password, confirm_password, name, phone_number):
        ...
        DbUtils().insert_into_table(table_name=USER_TABLE, column_names= "name, phone_number", values=f"'{name}','{phone_number}'")
        DbUtils().insert_into_table(table_name=LOGIN_TABLE, column_names="username,password" , values=f"'{username}','{password}'")
        return REGISTRATION_SUCCESS


if __name__ == "__main__":
    reg_control = RegistrationController()
    print(reg_control.registration(username="priyav", password="priyav123", confirm_password="priyav123",
                                   name="Priya Venkat", phone_number="7826776233"))
